...
const { loadTexture, loadJSON } = globalThis.isElectron
...
class WorldRenderer {
      ...
      worker.onmessage = ({ data }) => {
        if (data.type === 'geometry') {
          ...
          geometry.setAttribute(
            'position',
            new THREE.BufferAttribute(data.geometry.positions, 3),
          );
          geometry.setAttribute(
            'normal',
            new THREE.BufferAttribute(data.geometry.normals, 3),
          );
          geometry.setAttribute(
            'color',
            new THREE.BufferAttribute(data.geometry.colors, 3),
          );
          geometry.setAttribute(
            'uv',
            new THREE.BufferAttribute(data.geometry.uvs, 2),
          );
  ...
  setSectionDirty(pos, value = true) {
    ...
    this.sectionsOutstanding.add(
      `${Math.floor(pos.x / 16) * 16},${Math.floor(pos.y / 16) * 16},${
