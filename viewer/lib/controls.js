...
class MapControls {
  ...
  handleMouseMoveRotate(event) {
    ...
    this.rotateLeft(
      (2 * Math.PI * this.rotateDelta.x) / this.element.clientHeight,
    ); // yes, height
    this.rotateUp(
      (2 * Math.PI * this.rotateDelta.y) / this.element.clientHeight,
    );
      ...
      case 2:
        switch (this.touches.TWO) {
          case THREE.TOUCH.DOLLY_PAN:
            if (this.enableTouchZoom === false && this.enableTouchPan === false)
              return;
  ...
  onTouchMove(event) {
    ...
    switch (this.state) {
      ...
      case STATE.TOUCH_DOLLY_ROTATE:
        if (this.enableTouchZoom === false && this.enableTouchRotate === false)
          return;
