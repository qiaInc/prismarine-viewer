...
const parallel =
  ...
  process.env.CIRCLE_NODE_TOTAL !== undefined;
...
console.log({
  nodeIndex,
  nodeTotal,
  versions: viewer.supportedVersions.length,
});
const { firstVersion, lastVersion } = parallel
  ? testedRange(nodeIndex, nodeTotal, viewer.supportedVersions.length)
  : { firstVersion: 0, lastVersion: viewer.supportedVersions.length - 1 };
