...
class HeadlessViewer {
  ...
  updateMp4Frame(client) {
    ...
    imageStream.on('end', () => {
      this.idx++;
      if (this.idx < this.frames || this.frames < 0) {
        setTimeout(() => this.updateMp4Frame(client), 16);
      } else {
    ...
    getBufferFromStream(imageStream)
      .then(buffer => {
        ...
        if (client && client.writable) {
          ...
          this.idx++;
          if (this.idx < this.frames || this.frames < 0) {
            setTimeout(() => this.updateSocketFrame(client), 16); // 再帰
          } else {
            client.end();
          }
        }
